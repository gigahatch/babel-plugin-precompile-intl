...
export default function build(runtimeImportPath = "precompile-intl-runtime") {
  return declare<PrecompileIntlOptions>(
    ({ types: t, assertVersion }, options) => {
          ...
          if (Object.keys(entry.style.parsedOptions).length > 0) {
            ...
            let options = t.objectExpression(
              keys.map((key) => {
                if (!isNumberSkeleton(entry.style))
                  throw new Error(
                    "The entry should have had a number skeleton"
                  );
      ...
      function applyLiteralTransform(ast: MessageFormatElement[]) {
        if (!options.literalTransform) return;

        for (const el of ast) {
          switch (el.type) {
            case 0: // literal
              el.value = options.literalTransform(el.value) as string;
              continue;
      ...
      function flattenObjectProperties(
        ...
        propsArray: PropertyTuple[],
        currentPrefix?: string
      ) {
        return object.properties.forEach((op) => {
          if (!isObjectProperty(op))
            throw new Error(
              "Exported objects can only contain regular properties"
            );
          if (!isStringLiteral(op.key) && !isIdentifier(op.key))
            throw new Error(
              "Object keys for translations can only contain strings or identifiers"
            );
          ...
          if (t.isObjectExpression(op.value)) {
            ...
            let key =
              isStringLiteral(op.key) || !!currentPrefix
        ...
        visitor: {
          ...
          ObjectProperty({ node }) {
            if (t.isStringLiteral(node.value)) {
              ...
              t.isTemplateLiteral(node.value) &&
